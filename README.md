...
```bash
sudo apt update && sudo apt install -y nmap fping grc
```
...
- **权限**：运行脚本时建议使用 `sudo`，因为 `fping` 和某些 `nmap` 扫描（如 UDP 或全端口扫描）需要root权限。
- **防火墙**：部分扫描（如 `vuln` 或 UDP）可能被防火墙拦截，或对目标系统造成影响，请谨慎使用。
- **依赖检查**：若缺少 `grc`，脚本仍可运行，但输出不会高亮；若缺少 `nmap` 或 `fping`，脚本将退出并提示安装。
